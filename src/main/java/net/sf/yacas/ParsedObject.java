...
class ParsedObject {
	...
	void ReadToken() throws Exception {
		// Get token.
		iLookAhead = iParser.iTokenizer.NextToken(iParser.iInput, iParser.iEnvironment.HashTable());
			...
			if (iLookAhead == iParser.iEnvironment.iProgOpen.String()) {
				...
				if (op == null) {
					...
					if (LispTokenizer.IsSymbolic(iLookAhead.charAt(0))) {
						...
						while (len > 1) {
							len--;
							String lookUp = iParser.iEnvironment.HashTable().LookUp(iLookAhead.substring(0, len));
							...
							if (op != null) {
								...
								String lookUpRight = iParser.iEnvironment.HashTable().LookUp(toLookUp);
	...
	void Combine(int aNrArgsToCombine) throws Exception {
		...
		LispStandard.InternalReverseList(subList.Get().SubList().Get().Next(), subList.Get().SubList().Get().Next());
