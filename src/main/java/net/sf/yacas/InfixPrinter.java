...
class InfixPrinter extends LispPrinter {
	...
	public InfixPrinter(LispOperators aPrefixOperators, LispOperators aInfixOperators, LispOperators aPostfixOperators, LispOperators aBodiedOperators) {
	...
	void Print(LispPtr aExpression, LispOutput aOutput, int iPrecedence) throws Exception {
		...
		if (string != null) {
			...
			if (iPrecedence < KMaxPrecedence && string.charAt(0) == '-' && (LispTokenizer.IsDigit(string.charAt(1)) || string.charAt(1) == '.')) {
				bracket = true;
			}
			if (bracket)
				WriteToken(aOutput, "(");
