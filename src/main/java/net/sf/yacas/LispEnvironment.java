...
class LispEnvironment {
	...
	/**
	 * YacasArgStack implements a stack of pointers to objects that can be used to pass arguments to functions, and receive results back.
	 */
	class YacasArgStack {
	...
	void PushLocalFrame(boolean aFenced) {
		if (aFenced) {
			LocalVariableFrame newFrame = new LocalVariableFrame(iLocalsList, null);
			iLocalsList = newFrame;
		} else {
			LocalVariableFrame newFrame = new LocalVariableFrame(iLocalsList, iLocalsList.iFirst);
			iLocalsList = newFrame;
		}
	}
	...
	public LispUserFunction UserFunction(LispPtr aArguments) throws Exception {
		LispMultiUserFunction multiUserFunc = iUserFunctions.get(aArguments.Get().String());
		if (multiUserFunc != null) {
	...
	public void DefineRule(String aOperator, int aArity, int aPrecedence, LispPtr aPredicate, LispPtr aBody) throws Exception {
		// Find existing multiuser func.
		LispMultiUserFunction multiUserFunc = iUserFunctions.get(aOperator);
