...
class YacasEvaluator extends EvalFuncBase {
	...
	public YacasEvaluator(final YacasEvalCaller aCaller, final int aNrArgs, final int aFlags) {
		iCaller = aCaller;
		iNrArgs = aNrArgs;
		iFlags = aFlags;
	}
	...
	@Override
	public void Evaluate(final LispPtr aResult, final LispEnvironment aEnvironment, final LispPtr aArguments) throws Exception {
		if ((iFlags & Variable) == 0) {
			LispError.CheckNrArgs(iNrArgs + 1, aArguments, aEnvironment);
		}

		final int stacktop = aEnvironment.iStack.GetStackTop();
